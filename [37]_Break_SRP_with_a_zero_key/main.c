...
int main(int argc, char *argv[])
{
	...
	uint8_t zero_buf[SHA256_HASH_SIZE];
	...
	sha256_init(&zero_h);
	sha256_write(&zero_h, (uint8_t*) "0", 1);
	sha256_result(&zero_h,zero_buf);
	...
	email = argv[1];
	client_init(&c, N, g, k);
	salt_str = server_init_shared(&s, email, strlen(email), zero /*c.srp.pubkey*/);
	if (NULL != salt_str)
	{
		// HMAC test
		sha256_init_Hmac(&ctx, zero_buf, SHA256_HASH_SIZE);
		sha256_write(&ctx, (uint8_t*) salt_str, strlen(salt_str));
