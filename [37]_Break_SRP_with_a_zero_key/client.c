...
void client_init_shared(struct client_t *c, const char *password, size_t password_len, mpz_t server_pubkey)
{
	mpz_t u, x, S/*, salt*/;
	/*char *salt_str;*/
	...
	// 2. Generate x
	/*salt_str = mpz_get_str(NULL, 16, c -> salt);*/
	srp_utils_gen_x(&x, password, password_len, c -> salt_str, c -> salt_len);

	// 5. Generate S = (B - k * g**x)**(a + u * x) % N
	srp_utils_gen_client_shared_secret(&S, server_pubkey, c -> srp.g, c -> srp.k, x, u, c -> srp.id, c -> srp.N );
