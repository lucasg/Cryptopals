...
{
	size_t random_hexlen;
	char *hex_decrypted, *decrypted, *random_hexstr;
	mpz_t cipher[2], modulo, S, inv_S, result[2], e;

	mpz_init_set_str(e, NIST_e, 0 );

	// Capturing ciphertexts
	printf("Looking for RSA ciphertext generation. It can take a while ");
	if (rsa_encrypt(&(cipher[0]), &modulo, mc_solaar_secret, strlen(mc_solaar_secret)))
		return 0x01; // Error while encrypting
	printf("\nRSA encryption done.\n\n");

	// S random integer
	if (gen_random_hex_str(&random_hexstr, &random_hexlen, BITLEN_MIN/2, NULL))
		return 0x01;
	mpz_init_set_str(S, random_hexstr, 16);

	// Tampering ciphertext
	mpz_init(cipher[1]);
	mpz_powm(cipher[1], S, e, modulo);
	mpz_mul(cipher[1], cipher[1], cipher[0]);
	mpz_mod(cipher[1], cipher[1], modulo);

	// Decryption
	mpz_init(result[1]);
	mpz_powm(result[1], cipher[1], d, modulo);

	// Untampering
	mpz_init(inv_S);
	mpz_invmod(&inv_S, S, modulo);

	mpz_init(result[0]);
	mpz_mul(result[0], inv_S, result[1]);
	mpz_mod(result[0], result[0], modulo);

	hex_decrypted = mpz_get_str(NULL, 16, result[0]);
