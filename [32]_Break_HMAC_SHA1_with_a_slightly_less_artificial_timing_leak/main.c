...
int diff_cmp (const void * a, const void * b)
{
...
int main(void)
{
  ...
  size_t i, j,c, k, candidate;
  long /*diff,*/ total_diff, max_latency = 0, http_res;
  long diffs[10];
  ...
  printf("Warning : this exercice takes a long time to run (approx. 2000 sec or 30 mins).\n");
  ...
  if(curl) {
    ...
    for (i = 0; i < SHA1_HASH_LENGTH; i++)
    {
      ...
      for (j = 0x00; j < 0x101; j++)
      {
        ...
        /* Median Filtering */
        total_diff = 0;
        qsort((void*) diffs, 10, sizeof(*diffs), diff_cmp);
        for (k = 0; k < 10; k++)
        {
            total_diff += diffs[k] ;///(7-3);
        }

        if (j && total_diff > max_latency)
        {
          max_latency = total_diff;
          candidate = c;
        }
