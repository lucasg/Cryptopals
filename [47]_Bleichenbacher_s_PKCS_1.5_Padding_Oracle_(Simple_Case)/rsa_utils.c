...
int rsa_gen_random_hex_str(char **hexstring, size_t *hexlen, unsigned int minbitlen, struct mt19937_t *ext_gen)
{
	size_t i, min_charlen;
	...
	min_charlen = minbitlen/(8*sizeof(char));
...
int rsa_gen_prime_num(mpz_t *p, mpz_t *q, unsigned int bitlen_min)
{
	...
	if (rsa_gen_random_hex_str(&random_hexstr, &random_hexlen, bitlen_min, &r_gen))
		return 0x01;
